...
CHROME_DRIVER_PATH = os.path.join(BASE_PATH, 'drivers','chromedriver.exe')
IE_DRIVER_PATH = os.path.join(BASE_PATH, 'drivers','IEDriverServer.exe')
...
class Config:
    def __init__(self, config=CONFIG_FILE,case_data = CASE_PATH,
                 chrome = CHROME_DRIVER_PATH,ie = IE_DRIVER_PATH,screenshot = SCREENSHOTS_PATH):
        self.config = YamlReader(config).data
        self.case_data = YamlReader(case_data).data
        # self.interface_data = YamlReader(interface_data).data
        # self.exe_ptah = EXE_PATH
        self.chrome_driver_pt = chrome
        self.ie_driver_pt = ie
    ...
    def get_chrom_driver_path(self):
        return self.chrome_driver_pt

    def get_ie_driver_path(self):
        return self.ie_driver_pt
    
# if __name__ == "__main__":
#     c = Config()
#     print(c.get_ele('locator').get('search_input'))   