...
class BasePage():
    ...
    def get_page_title(self):
        logger.info("当前页面的title为: {}".format(self.driver.title))
    ...
    def find_element(self, *loc):
        try:
            ...
            logger.warning('找不到定位元素: {}'.format(loc[1]))
            raise
    ...
    def send_key(self, *loc, text):
        logger.info('清空文本框内容: {}...'.format(loc[0]))
    ...
    def click(self, loc):
        logger.info('点击元素 by {}: {}...'.format(loc[0], loc[1]))
        try:
    ...
    def get_cssValue(self,*loc,cssKey):
        '''获取对于css属性值'''
        element = self.find_element(*loc)
        return element.value_of_css_property(cssKey)
