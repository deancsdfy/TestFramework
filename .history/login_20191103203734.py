...
class Page(unittest.TestCase):
    ...
    def test_login_errorPassword(self):
        ...
        self.accountMsg = self.driver.find_element_by_class_name('account-form-error').text
        logger.info('页面跳转成功')
    ...
    def test_downloadAppPanelisDisable(self):
        lnkDownloadApp =  WebDriverWait(self.driver,20,0.2).until(lambda x:x.find_element_by_link_text('下载豆瓣客户端'))
        ActionChains(self.driver).move_to_element(lnkDownloadApp).perform()
        doubanappCssValue = self.driver.find_element_by_class_name('top-nav-doubanapp').value_of_css_property('display')
